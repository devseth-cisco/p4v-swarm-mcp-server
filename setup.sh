...
#  10. Shell env          (~/.zshrc.d/p4-mcp.sh so Cursor terminals get P4 vars)
#
...
# ═══════════════════════════════════════════════════════════════════════════════
# Step 10: Shell env snippet (so Cursor terminals have p4 env)
# ═══════════════════════════════════════════════════════════════════════════════
step "10" "Installing shell environment..."

ZSHRC_DIR="$HOME/.zshrc.d"
mkdir -p "$ZSHRC_DIR"
ENV_FILE="$ZSHRC_DIR/p4-mcp.sh"

cat > "$ENV_FILE" << ENVEOF
# Perforce environment (auto-generated by p4v-swarm-mcp-server/setup.sh)
export P4PORT="$P4PORT"
export P4USER="$P4USER"
export P4TICKETS="$P4_TICKETS"
export P4_BIN="$P4_BIN"
export PATH="$P4_DIR:\$PATH"
ENVEOF

ZSHRC="$HOME/.zshrc"
HOOK='for f in ~/.zshrc.d/*.sh(N); do source "$f"; done'
if ! grep -qF '.zshrc.d' "$ZSHRC" 2>/dev/null; then
    echo "" >> "$ZSHRC"
    echo "# Auto-source ~/.zshrc.d snippets" >> "$ZSHRC"
    echo "$HOOK" >> "$ZSHRC"
    ok "Added ~/.zshrc.d sourcing hook to ~/.zshrc"
fi
ok "Shell env: $ENV_FILE"
